...
used if converted to a Set with entrySet.)
        ...
        prinElements(nums);
        prinElements2(nums);
        prinEvenNumbers1(nums);
        prinEvenNumbers2(nums);
    ...
    public static void prinEvenNumbers1(List<Integer> nums){

        for (Integer w:nums  ) {
            if (w%2==0) System.out.println("w = " + w);
        }

    }
    public static void prinEvenNumbers2(List<Integer> nums){
